...
# Remove single device detection
...
daily_step = data.query("by=='both'").groupby(["user","day","device"]).agg(step = ("step","sum"))
...
Phone, Watch, Phone-Watch Boxplot for Daily Step after removing single device detection
''')
fig.supylabel("Step Counts")

plt.savefig("../Figure/006.png")