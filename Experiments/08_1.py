...
datas = getDayNightRatio(users, df)
datas = [data.sort_index(level = 0, key=lambda x: [users.index(user) for user in x.to_numpy()]) for data in datas]

color = {"day":"tab:olive", "night":"tab:brown"}
for idx, btype in enumerate(["phone","watch"]):
    for jdx, day in enumerate(["day","night"]):
        ax[idx].bar(x=np.arange(n_user)*2.5 + .5 + jdx, height = datas[jdx][btype[0]].to_numpy() + datas[jdx]['b'].to_numpy(), label = day , color = color[day])
...
fig.supxlabel("User\n\nComparision of Covering Ratio between night and day")
