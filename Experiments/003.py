...
def getPlotlyGraph(name, input_timestamp):
    ...
    step_count = df.query(f"user == {uid}")
    ...
    for idx, row in watch_step.iterrows():
        hovertemplate.append(f"Time: {row['timestamp']} <br>Step: {row['step']}")
    overall.append_trace(go.Bar(x=watch_step["timestamp"], y= watch_step["step"], orientation='v', showlegend=True, hovertemplate = hovertemplate, name="watch"), 2, 1)
