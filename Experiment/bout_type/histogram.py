...
ax[0].bar(x= list(np.arange(0.5,20.5-1)), height = data[2,:], width = .8, bottom = data_cum[1],label='both', color = color[2])
ax[0].bar(x= list(np.arange(0.5,20.5-1)), height = data[1,:], width = .8, bottom = data_cum[0],label='watch', color = color[1])
ax[0].bar(x= list(np.arange(0.5,20.5-1)), height = data[0,:], width = .8, bottom = data_cum[0]-data[0],label='phone', color = color[0])
ax[0].set_xticks(np.arange(0.5,20.5-1))
ax[0].set_xticklabels(np.arange(0.5*50,(20.5-1)*50,50), fontsize =7)
...
data_cum = data.cumsum(axis=0)
ax[1].bar(x= list(np.arange(0.5,24.5-1)), height = data[2,:], width = .8, bottom = data_cum[1,:],label='both', color = color[2])
ax[1].bar(x= list(np.arange(0.5,24.5-1)), height = data[1,:], width = .8, bottom = data_cum[0,:],label='watch', color = color[1])
ax[1].bar(x= list(np.arange(0.5,24.5-1)), height = data[0,:], width = .8, bottom = data_cum[0,:]-data[0,:],label='phone', color = color[0])
ax[1].legend()
ax[1].set_xticks(np.arange(0.5,24.5-1))
ax[1].set_xticklabels(np.arange(1,24))
ax[1].set_xlabel('Duration')
plt.savefig(os.path.join(cwd,'Fig','bout_type_historgram.png'))
