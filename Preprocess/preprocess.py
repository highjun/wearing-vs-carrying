...
for idx, folder in enumerate(sorted(os.listdir(raw_dir))):
    raw = pd.read_csv(glob.glob(os.path.join(raw_dir,f"{folder}/**/com.samsung.shealth.tracker.pedometer_step_count.*.csv"), recursive= True)[0],
                                        skiprows=[0], usecols = [
                                            ord("C")-ord("A"),
                                            ord("D")-ord("A"),
                                            ord("E")-ord("A"),
    ...
    device_profile = pd.read_csv(glob.glob(os.path.join(raw_dir,f"{folder}/**/com.samsung.health.device_profile.*.csv"), recursive= True)[0],
