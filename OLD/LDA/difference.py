...
def get_coarse_level(time):
    if time < 7: 
        ...
        return [21,24]
hourly['word'] = [get_active_level(*steps)[1 if n_exp =="phone" else 2] for steps in hourly[['b','p','w']].to_numpy()]
...
def display_topics(transform, n_exp, nrows = 2, ncols = 4):
    ...
    for idx,ax in enumerate(axes.flatten()):
        ...
        if n_exp == 'phone':
            clist = [mcolors.hsv_to_rgb(hsv) for hsv in [(200/360, 0,1), (200/360,0.5,1), (200/360,1,1)]]
        elif n_exp =='watch':
            clist = [mcolors.hsv_to_rgb(hsv) for hsv in [(30/360, 0,1), (30/360,0.5,1), (30/360,1,1)]]
...
display_topic_word_distribution(LDA, transform)
