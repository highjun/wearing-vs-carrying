...
def get_feature(row):
    if row[columns.index('totalcount')] <= 1 and row[columns.index('totalstep')] <10:
        ...
        return 2
    elif row[columns.index('wstep_ratio')] <=.05:
        return 3
    else:
        return 4
...
def display_topics(transform, n_exp):
    fig, axes = plt.subplots(nrows  = K//4, ncols = 4, figsize = (4*4.8, 3*3.2))
