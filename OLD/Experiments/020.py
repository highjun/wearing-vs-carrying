...
tmp = tmp_hour.groupby("weekday").agg(phone=("p_act","sum"), watch = ("w_act","sum"), both = ("b_act","sum"))
fig, ax = plt.subplots(nrows = 1, ncols = 1, constrained_layout = True)
plt.plot(range(7), tmp.values[:,0]/(tmp.values[:,0]+tmp.values[:,2]),label = "phone")
plt.plot(range(7), tmp.values[:,1]/(tmp.values[:,1]+tmp.values[:,2]),label = "watch")
